...
##########################################################
#注意：请将此脚本放置于源码根目录下
#参考：http://blog.fpliu.com/it/software/development/language/C/library/libvorbis/build-for-android-with-cmake
##########################################################

#如果您已经编译好libogg，请指定它的安装目录
#如果您没有编译好libogg，我们会自动下载libogg的源码进行编译
...
build() {
    ...
    -DOGG_INCLUDE_DIRS=${LIB_OGG_NDK_BUILD_DIR}/${TARGET_ABI}/include \
    -DOGG_LIBRARIES=${LIB_OGG_NDK_BUILD_DIR}/${TARGET_ABI}/lib/libogg.so \
...
main() {
    ...
    do
        info "Building libvorbis for ${TARGET_ABI}"
        build || exit
    done
    build_success
}
